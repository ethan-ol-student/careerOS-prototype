...
const Schema = z.object({ scope: z.enum(["candidate", "employer", "all"]) });

/**
 * POST /api/dev/reset — wipe test data for the test account. Dev only.
 * Scoped strictly to the test account's profiles, so real users are
 * never touched.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!parsed.success) {
      return failFromCode("validation", "scope must be candidate, employer, or all.");
    }
